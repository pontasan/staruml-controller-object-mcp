# StarUML Controller MCP Server — Object Diagram

An MCP (Model Context Protocol) server specialized for **Object Diagrams**. Enables AI assistants like Claude to programmatically create and manage object diagrams — objects (with slots) and links in [StarUML](https://staruml.io/).

<p align="center">
  <img src="images/main.jpg" alt="Object Diagram Example — EC Order Snapshot created entirely via MCP" width="800">
</p>

## Architecture

<p align="center">
  <img src="images/architecture.svg" alt="Architecture: Claude ↔ MCP (stdio) ↔ staruml-controller-object-mcp ↔ HTTP REST API ↔ StarUML" width="800">
</p>
...
```bash
git clone https://github.com/pontasan/staruml-controller-object-mcp.git
cd staruml-controller-object-mcp
npm install && npm run build
```
...
4. The HTTP server starts — all Object Diagram tools become available via MCP
...
```json
{
  "mcpServers": {
    "staruml-object": {
      "command": "node",
      "args": ["/absolute/path/to/staruml-controller-object-mcp/dist/index.js"]
    }
  }
}
```
...
```bash
claude mcp add staruml-object node /absolute/path/to/staruml-controller-object-mcp/dist/index.js
```
...
### Object Diagram Tools

The core of this server — full CRUD for every Object Diagram resource.

| Resource | Tools |
|---|---|
| **Diagrams** | `object_list_diagrams`, `object_create_diagram`, `object_get_diagram`, `object_update_diagram`, `object_delete_diagram` |
| **Objects** | `object_list_objects`, `object_create_object`, `object_get_object`, `object_update_object`, `object_delete_object` |
| — Slots | `object_list_object_slots`, `object_create_object_slot` |
| **Links** | `object_list_links`, `object_create_link`, `object_get_link`, `object_update_link`, `object_delete_link` |
